...
AI reports are generated by either OpenAI or AWS Bedrock (Claude), selected via `LLM_PROVIDER`:

- `LLM_PROVIDER=openai` (default) — uses `OPENAI_API_KEY` and `OPENAI_MODEL` (default `gpt-5.5`).
- `LLM_PROVIDER=bedrock` — calls Claude on Amazon Bedrock via `AWS_REGION` and `BEDROCK_MODEL_ID`
  (default `anthropic.claude-opus-4-8`). AWS credentials are resolved from the standard chain
  (environment variables, shared profile, or IAM role) — do not put AWS keys in `.env`.
