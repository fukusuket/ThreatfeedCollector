...
AI reports are generated by either OpenAI or AWS Bedrock, selected via `LLM_PROVIDER`:

- `LLM_PROVIDER=openai` (default) — uses `OPENAI_API_KEY` and `OPENAI_MODEL` (default `gpt-5.5`).
- `LLM_PROVIDER=bedrock` — calls models on Amazon Bedrock via `AWS_REGION` and `BEDROCK_MODEL_ID`
  (default `openai.gpt-5.5`).
  - GPT-5.x models (`openai.gpt-5.5` etc.) use the `bedrock-mantle` Responses API and require a
    Bedrock API key in `AWS_BEARER_TOKEN_BEDROCK`; they are available only in `us-east-1` / `us-east-2`.
  - Anthropic and gpt-oss models use `InvokeModel`; AWS credentials are resolved from the standard
    chain (environment variables, shared profile, or IAM role) — do not put AWS keys in `.env`.
