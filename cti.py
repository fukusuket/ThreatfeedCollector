...
def create_misp_event(misp: PyMISP, article: Dict, iocs: Dict[str, Set[str]]) -> bool:
    ...
    try:
        ...
        ai_summary = analyze_threat_article(article_text=article['content'], model="gpt-5.2", article_title=event_title, article_url=article['url'])
        event.add_event_report(name="[en]_[gpt-5.2]_" + event_title, content=trim_markdown_fence(ai_summary), distribution=0)
        ai_summary = analyze_threat_article(article_text=article['content'], model="gpt-5.2", article_title=event_title, article_url=article['url'], additional_pre_context="Please provide the final output as a Japanese translation.")
        event.add_event_report(name="[jp]_[gpt-5.2]_" + event_title, content=trim_markdown_fence(ai_summary), distribution=0)
        ai_summary = analyze_threat_article(article_text=article['content'], model="gpt-4o", article_title=event_title, article_url=article['url'])
