...
def fetch_url_content(url: str) -> str:
    ...
    try:
        if not url:
            return ""
        response = requests.get(url, timeout=10, verify=False, headers={'User-Agent': feedparser.USER_AGENT})
...
def process_feed(vendor_name: str, feed_url: str, cutoff_date: datetime) -> List[Article]:
    try:
        ...
        response = requests.get(feed_url, timeout=10, verify=False, headers={'User-Agent': feedparser.USER_AGENT})
        response.raise_for_status()
...
def fetch_full_content(article: Article, crawl_links: bool = False, max_links: int = 30, crawl_same_domain: bool = False) -> List[Article]:
    url = article.get('url', '')
    ...
    try:
        ...
        for link_tag in soup.find_all('a', href=True):
            ...
            if not full_url.lower().startswith(('http://', 'https://')) or full_url in seen_urls:
                continue
            ...
            try:
                ...
                r = requests.get(full_url, timeout=10, verify=False, headers={'User-Agent': feedparser.USER_AGENT})
                r.raise_for_status()
...
def add_event_to_misp(article: Article, iocs: Dict, misp: PyMISP) -> bool:
    ...
    try:
        ...
        if existing_events and len(existing_events) > 0:
            ...
            return False
        url_attrs = misp.search(controller="attributes", value=article.get('url', ''), type="url", category="External analysis", pythonify=True)
...
def save_stats(misp: PyMISP) -> None:
    try:
        date_from = (datetime.now() - timedelta(days=int(DAYS_BACK))).strftime('%Y-%m-%d')
        date_to = datetime.now().strftime('%Y-%m-%d')
        events = misp.search(date_from=date_from, date_to=date_to, pythonify=True)

        with open(OUTPUT_CSV, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=['date', 'vendor', 'iocs', 'title', 'blog url'])
            writer.writeheader()
            for event in events:
                vendor = event.info.split(']')[0].strip('[').split(']')[0] if ']' in event.info else 'Unknown'
                ioc_count = len(event.attributes) - 1
                title = re.sub(r'^\[.*?\]\s*', '', event.info)
                for attr in event.attributes:
                    if attr.category == 'External analysis' and attr.type == 'url':
                        writer.writerow({'date': event.date, 'vendor': vendor, 'iocs': ioc_count, 'title': title, 'blog url': attr.value})
                        break
        logger.info(f"Stats saved to {OUTPUT_CSV}")
        logger.info(f"Total: {sum(len(e.attributes) - 1 for e in events)} IOCs, {len(events)} events created")
    except Exception as e:
...
def main() -> None:
    ...
    for vendor, feed_url, blog_url, crawl_links in feeds:
        ...
        if feed_url:
            ...
            articles = [{'title': f"{vendor} blog", 'date': datetime.now().strftime('%Y-%m-%d'),'url': blog_url, 'content': '', 'vendor': vendor}]
            crawl_same_domain = True

        for article in articles:
            process_article(misp, article, vendor, should_crawl_links, crawl_same_domain)
